/* ─────────────────────────────────────────────────────────────────────────
   ...
   Цифры настоящие: тянем ../../live.json, тот же файл кормит боевой портал.
   Механика перенесена из лаборатории: док с каплей, разворот плитки,
   кольца свежести, набегающие цифры.
   ───────────────────────────────────────────────────────────────────────── */
(function () {
  'use strict';
  ...
  // порядок и оформление плиток; group — для фильтров
  var REPORTS = [
    { key: 'finance',    href: '../../finance.html',            name: 'БДДС',        icon: 'coin',  skin: 'olive', group: 'money',  slots: [600, 1080] },
    { key: 'sales',      href: '../../sales.html',              name: 'Продажи',     icon: 'chart', skin: 'rose',  group: 'money',  slots: [600, 1080] },
    { key: 'forecast',   href: '../../forecast.html',           name: 'Прогноз',     icon: 'wave',  skin: 'sand',  group: 'guests', slots: [600, 1080] },
    { key: 'attendance', href: '../../attendance.html',         name: 'Посещения',   icon: 'bars',  skin: 'clay',  group: 'guests', slots: [620, 1100] },
    { key: 'loyalty',    href: '../../loyalty.html',            name: 'Лояльность',  icon: 'heart', skin: 'sage',  group: 'guests', slots: 'hourly' },
    { key: 'staff',      href: '../../staff.html',              name: 'ФОТ и штат',  icon: 'users', skin: 'slate', group: 'people', slots: [600, 1080] }
  ];
  ...
  /* ── форматирование ──────────────────────────────────────────────────── */
  function nf(v, d) {
    var p = (typeof v === 'number' ? v.toFixed(d == null ? 0 : d) : String(v)).split('.');
    p[0] = p[0].replace(/\B(?=(\d{3})+(?!\d))/g, ' ');
    return p.join(',');
  }
  function pct(v) { return (v > 0 ? '+' : '') + nf(v, 1).replace('-', '−') + '%'; }
  function hhmm(iso) {
    if (!iso) return '—';
    var d = new Date(iso);
    if (isNaN(d)) return '—';
  ...
  /* ── герой: выполнение плана месяца ──────────────────────────────────── */
  function paintHero() {
    var pf = (L.attendance || {}).planfact;
    var el = document.getElementById('planPct');
    var cap = document.getElementById('planCap');
    var delta = document.getElementById('planDelta');
    if (!pf || pf.pct == null) { cap.textContent = 'плана месяца · нет данных'; return; }
    ...
    // Маркер на шкале. Кусочно-линейно, чтобы 90% и 100% попадали ровно на стыки
    // полос, а не «примерно туда» — иначе шкала врёт.
    var p = pf.pct, t;
    if (p <= 90)       t = clamp((p - 75) / 15, 0, 1) / 3;
    else if (p <= 100) t = 1 / 3 + (p - 90) / 10 / 3;
    else               t = 2 / 3 + clamp((p - 100) / 15, 0, 1) / 3;
    requestAnimationFrame(function () {
      document.getElementById('bandsMarker').style.left = (t * 100).toFixed(1) + '%';
    });
  }
  ...
  function paintChart() {
    ...
    var data = rev.slice(-14);
    var W = 100, H = 100, PAD = 6;
    ...
    // вторая линия — настоящие гости за те же дни, приведённые к шкале выручки
    var ghost = null;
    ...
    var peak = data.indexOf(mx);
    var px = xs(peak), py = ys(mx);
    ...
    // подпись и «пузырь» рисуем отдельным слоем без растяжения по X
  ...
  function tileData(r) {
    ...
    if (r.key === 'finance') {
      ...
      d.side = nf(f.kassaRub / 1e3, 0) + 'к'; d.sideCap = 'касса';
    ...
    } else if (r.key === 'attendance') {
      d.value = pct(a.yoy12); d.unit = '';
    ...
    } else if (r.key === 'loyalty') {
      // числа по лояльности в live.json не приезжают — показываем свежесть, а не выдумываем
      d.value = hhmm(st.lastUpdate); d.unit = '';
      d.label = 'последнее обновление · каждый час';
      d.side = '1 ч'; d.sideCap = 'цикл';
      d.rows = [['Обновляется', 'каждый час'], ['Последнее', hhmm(st.lastUpdate)],
                ['Дашборд', '@BadenLoyalty_bot']];
    } else if (r.key === 'staff') {
      ...
      d.rows = c.slice(0, 3).map(function (x) {
        return [x.name, nf(x.filled, 1) + '% · дефицит ' + x.deficit, x.filled < 60];
      });
    }
    return d;
  }

  // сколько прошло от прошлого слота обновления до следующего
  function freshness(r, lastUpdate) {
    ...
    return {
      p: clamp((mins - prev) / (next - prev), 0, 1),
      stale: (mins - luMin) > (next - prev) + 60
    };
  }
  ...
  function buildTiles() {
    ...
    REPORTS.forEach(function (r) {
      ...
      a.innerHTML =
        ...
        '<div class="tile__foot"><span>Обновлено</span><b>' + hhmm(st.lastUpdate) + '</b></div>';
    ...
    document.getElementById('alertCount').textContent = warn;
    tilt();
  }

  // наклон с инерцией — тот же приём, что на боевом портале
  function tilt() {
  ...
  function expand(tile, r, d) {
    ...
    var w = Math.min(620, innerWidth - 28);
    var left = (innerWidth - w) / 2;
    ...
    sheet.innerHTML =
      '<div class="sheet__top">' +
        ...
        '<h3>' + r.name + '</h3>' +
        '<button class="ico ico--soft" type="button" aria-label="Закрыть">' +
  ...
  fetch('../../live.json?t=' + Date.now())
    ...
    .catch(function (e) {
      document.getElementById('tiles').innerHTML =
        '<div style="padding:24px;color:var(--ink-2)">Не удалось загрузить live.json — ' + e.message + '</div>';
    });
})();
